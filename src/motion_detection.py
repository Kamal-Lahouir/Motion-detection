...
def motion_detection():
    ...
    while True:
        # gives 2 outputs retval,frame - [1] selects frame
        ...
        # and we need to specify the standerd devation in x and y direction which is the (0) if only x(sigma x) is specified
        # then y(sigma y) is taken as same as x. sigma = standerd deveation(mathmetics term)

        # uses a kernal of size(5,5)(width,height) which goes over 5x5 pixel area left to right
        ...
        dilate_image = cv2.dilate(thresh, None, iterations=15)
        ...
        cnt, a = cv2.findContours(
            dilate_image.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        ...
        for c in cnt:
            # if contour area is less then 800 non-zero(not-black) pixels(white)
            if cv2.contourArea(c) > 1200:
                # x,y are the top left of the contour and w,h are the width and hieght
                (x, y, w, h) = cv2.boundingRect(c)

                # (0, 255, 0) = color R,G,B = lime / 2 = thickness(i think?)(YES IM RITE!)
        ...
        #   (10, frame.shape[0] - 10) , 0.35, (0, 0, 255),1) # frame.shape[0] = hieght, frame.shape[1] = width,ssssssssssssss
        ...
        # (1) = time delay in seconds before execution, and 0xFF takes the last 8 bit to check value or sumin
